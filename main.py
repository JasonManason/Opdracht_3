...
def create_new_table(table):
    ...
    Creates new tables for every different kind of recommendation if table does not already exist.
        ...
        :return: None.
    """
    sql = ("CREATE TABLE IF NOT EXISTS rec_%s (id VARCHAR PRIMARY KEY, promo VARCHAR, product_name VARCHAR, sub_sub_category VARCHAR, target_audience VARCHAR, price int);"%(table))
    cur.execute(sql)

    # sql = ("DROP TABLE IF EXISTS rec_%s;"
    #        "CREATE TABLE IF NOT EXISTS rec_%s (id VARCHAR PRIMARY KEY, promo VARCHAR, product_name VARCHAR, sub_sub_category VARCHAR, target_audience VARCHAR, price int);"%(table, table))
    # cur.execute(sql)
